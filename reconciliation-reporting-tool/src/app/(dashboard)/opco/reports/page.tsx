...
export default function OpCoReportsPage() {
  ...
  const canUpload = useMemo(
    () => !!file && !!serviceId && !!opcoId && !busy,
    [busy, file, opcoId, serviceId],
  );
  ...
  async function loadMasters() {
    ...
    if (a?.ok) setOpCos(a.opcos);
  }

  async function loadReports(nextOpcoId: string) {
    const params = new URLSearchParams();
    params.set("type", "OPCO_MONTHLY");
    if (nextOpcoId) params.set("opcoId", nextOpcoId);
  ...
  useEffect(() => {
    if (!opcoId && opcos.length > 0) setOpCoId(opcos[0]!.id);
  }, [opcos, opcoId]);

  useEffect(() => {
    if (!opcoId) return;
    void loadReports(opcoId).catch((e) =>
  ...
  async function upload(e: React.FormEvent) {
    ...
    if (!opcoId) return setError("OpCo assignment is required");

    setBusy(true);
    setError(null);
    try {
      const meta = {
        type: "OPCO_MONTHLY" as const,
        month,
        year,
        serviceId,
        opcoId,
  ...
  return (
    <div className="space-y-2">
      ...
      <p className="text-sm text-zinc-600">Upload and view OpCo Monthly reports.</p>
          ...
          <form className="mt-4 space-y-3" onSubmit={upload}>
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="space-y-1">
                ...
                >
                  {opcos.length === 0 ? (
                    <option value="">No OpCo assignments</option>
                  ) : null}
                  {opcos.map((o) => (
        ...
        <div className="rounded-xl border bg-white p-5">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              onClick={() =>
                void loadReports(opcoId).catch(() => setError("Failed to refresh"))
              }
              disabled={!opcoId}
            >
              Refresh
